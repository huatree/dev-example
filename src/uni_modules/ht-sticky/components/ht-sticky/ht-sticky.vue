<template>
    <view class="ht-sticky" :id="ht_Id">
        <!-- 容器 -->
        <view :style="{ height: stickyHeight, background: backgroundColor }" v-if="isFixed"></view>
        <view :class="{ 'ht-sticky-fixed': isFixed }" :style="{ top: isFixed ? stickyTop + 'px' : 'auto' }">
 ...
 * @property {Boolean} isNativeHeader 是否是原生自带header
 * @property {String} stickyHeight 吸顶容器 高度 rpx
 * @property {String} backgroundColor 占位容器背景颜色
 * @property {Number} recalc 是否重新计算[有异步加载时使用,设置大于0的数]
 * @property {Number | String} index 列表中的索引值
 * @event {Function} sticky
...
export default {
    ...
    data() {
        const Id = `ht_${Math.ceil(Math.random() * 10e5).toString(36)}`;
        return {
            timer: null,
            top: 0,
            height: 0,
